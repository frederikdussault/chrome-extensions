...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function fetchSite(site, callback) {
    ...
    fetch(myRequest)
      ...
      .catch( (error) => {
        console.error(error);
        callback(file, `Message: Is it a valid URL?<br>ERROR: ${error.name}: ${error.message}; Response code: ${res.status}`);
      });
  }
  ...
  `;
    ...
    newRow.setAttribute('class', (status === 'good') ? 'good' : 'error'); // TODO test
